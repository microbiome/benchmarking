...
benchmark_out <- bench::press(
  N = N,
  {
    ...
    if( N <= beta_threshold ){
      ...
      # Use dummy expressions because all results must have the same length
...
benchmark_df <- benchmark_out %>%
  ...
  filter(method != "beta" | N <= beta_threshold)

# Write to file
write.csv(as.data.frame(benchmark_df),
          file = "article/benchmark_rawdata.csv",
          row.names = FALSE)
...
write.csv(benchmark_df,
          file = "article/benchmark_results.csv",
          row.names = FALSE)
