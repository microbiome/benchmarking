...
df <- df %>% mutate(Agglomerate = rep(NA, 2 * len_set * len_N),
                    AgglomerateCommand = c(rep("mia_agglomerateByRank", len_set * len_N), rep("phyloseq_tax_glom", len_set * len_N)))
```

```{r experiment_functions, echo = FALSE}
# test agglomeration for tse
agg_tse_exec_time <- function(tse) {
...
agg_pseq_exec_time <- function(pseq) {
...
for (data_set in data_sets) {
  ...
  for (N in sample_sizes) {
    ...
    if (ncol(tse) >= N) {
      ...
      df$Agglomerate[tse_ind] <- agg_tse_exec_time(sub_tse)

      # test melting for pseq
      df$Agglomerate[pseq_ind] <- agg_pseq_exec_time(sub_pseq)
...
p1 <- ggplot(filter(df, Samples == 10), aes(x = Features, y = Agglomerate, color = AgglomerateCommand)) +
  geom_point() + 
  geom_line() +
  labs(title = paste("Agglomeration comparison (N=", paste(unique(df$Samples), collapse=","), ")"),
       x = "Features (D)",
       y = "Execution time (s)",
       color = "Method:",
...
df2 <- pivot_wider(filter(df, Samples == 10)[ , c("Dataset", "Agglomeration", "Features", "ObjectType")] %>%
         filter(!is.na(Agglomerate)), names_from=c(ObjectType),
	        values_from=Agglomerate, Features) %>%
