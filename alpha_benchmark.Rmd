...
df <- df %>% mutate(AlphaEstimation = rep(NA, 2 * len_set * len_N),
                    AlphaCommand = c(rep("mia_estimateDiversity", len_set * len_N), rep("microbiome_diversity", len_set * len_N)))
```
...
for (data_set in data_sets) {
  ...
  for (N in sample_sizes) {
    ...
    if (ncol(tse) >= N) {
      ...
      df$AlphaEstimation[tse_ind] <- alpha_tse_exec_time(sub_tse)

      # test melting for pseq
      df$AlphaEstimation[pseq_ind] <- alpha_pseq_exec_time(sub_pseq)
...
p1 <- ggplot(filter(df, Samples == 10), aes(x = Features, y = AlphaEstimation, color = AlphaCommand)) +
  geom_point() + 
  geom_line() +
  labs(title = paste("Alpha estimation comparison (N=", paste(unique(df$Samples), collapse=","), ")"),
       x = "Features (D)",
       y = "Execution time (s)",
       color = "Method:",
       caption = "Execution time of alpha diversity estimation as a function of number of features") +
...
```{r beta_features_ratio}
df2 <- pivot_wider(filter(df, Samples == 10)[ , c("Dataset", "Alpha Estimation", "Features", "ObjectType")] %>%
         filter(!is.na(AlphaEstimation)), names_from=c(ObjectType),
	        values_from=AlphaEstimation, Features) %>%
