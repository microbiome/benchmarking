...
data_sets <- c("AsnicarF_2017.relative_abundance", "GlobalPatterns", "VincentC_2016", "SilvermanAGutData", "SprockettTHData")
...
assay_values <- rep(c("relative_abundance", "counts", "relative_abundance", "counts", "counts"), 2)
...
Execution times for different experiments, data sets and containers are evaluated with a recursive approach. Results are then stored into the matrices `diff_tse` and `diff_pseq`, which contains the execution times of tse-based and pseq-based experiments, respectively.
...
condition_6 <- data_set != "SilvermanAGutData"
```
...
for (data_set in data_sets) {
  ...
  if (condition_1[cur_set]) {
    ...
    tse <- ifelse((condition_5[cur_set]), tmp[[1]], tmp[[6]])
