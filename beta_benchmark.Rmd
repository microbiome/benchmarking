...
df <- df %>% mutate(BetaEstimation = rep(NA, 2 * len_set * len_N),
                    BetaCommand = c(rep("runMDS", len_set * len_N), rep("ordinate", len_set * len_N)))
```
...
for (data_set in data_sets) {
  ...
  for (N in sample_sizes) {
    ...
    if (ncol(tse) >= N) {
      ...
      df$BetaEstimation[tse_ind] <- beta_tse_exec_time(sub_tse)

      # test melting for pseq
      df$BetaEstimation[pseq_ind] <- beta_pseq_exec_time(sub_pseq)
...
p1 <- ggplot(filter(df, Samples == 10), aes(x = Features, y = BetaEstimation, color = BetaCommand)) +
  geom_point() + 
  geom_line() +
  labs(title = paste("Beta estimation comparison (N=", paste(unique(df$Samples), collapse=","), ")"),
       x = "Features (D)",
       y = "Execution time (s)",
       color = "Method:",
       caption = "Execution time of beta diversity estimation as a function of number of features") +
...
df2 <- pivot_wider(filter(df, Samples == 10)[ , c("Dataset", "Beta Estimation", "Features", "ObjectType")] %>%
         filter(!is.na(BetaEstimation)), names_from=c(ObjectType),
	        values_from=BetaEstimation, Features) %>%
