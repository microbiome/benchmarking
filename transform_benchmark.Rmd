...
df <- df %>% mutate(Transform = rep(NA, 2 * len_set * len_N),
                    TransformCommand = c(rep("mia_transformSamples", len_set * len_N), rep("microbiome_transform", len_set * len_N)))
```

```{r experiment_functions, echo = FALSE}
# test transformation for tse
trans_tse_exec_time <- function(tse) {
...
trans_pseq_exec_time <- function(pseq) {
...
for (data_set in data_sets) {
  ...
  for (N in sample_sizes) {
    ...
    if (ncol(tse) >= N) {
      ...
      df$Transform[tse_ind] <- trans_tse_exec_time(sub_tse)

      # test melting for pseq
      df$Transform[pseq_ind] <- trans_pseq_exec_time(sub_pseq)
...
p1 <- ggplot(filter(df, Samples == 10), aes(x = Features, y = Transform, color = TransformCommand)) +
  geom_point() + 
  geom_line() +
  labs(title = paste("Transformation comparison (N=", paste(unique(df$Samples), collapse=","), ")"),
       x = "Features (D)",
       y = "Execution time (s)",
       color = "Method:",
...
df2 <- pivot_wider(filter(df, Samples == 10)[ , c("Dataset", "Transformation", "Features", "ObjectType")] %>%
         filter(!is.na(Transform)), names_from=c(ObjectType),
	        values_from=Transform, Features) %>%
