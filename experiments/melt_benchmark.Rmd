...
for (data_set in data_sets) {
  ...
  for (N in sample_sizes) {
    ...
    if (ncol(tse) >= N) {
      ...
      # store assay name
      df$AssayValues[tse_ind] <- assayNames(sub_tse)
      df$AssayValues[pseq_ind] <- assayNames(sub_tse)
...
df2 <- pivot_wider(filter(df, Samples == 100)[ , c("Dataset", "Melt", "Features", "ObjectType")] %>%
