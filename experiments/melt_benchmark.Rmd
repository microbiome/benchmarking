...
df <- df %>% mutate(MeltTime = rep(NA, 2 * len_R * len_set * len_N),
                    MeltCommand = rep(c(rep("mia::meltAssay", len_set * len_N), rep("phyloseq::psMelt", len_set * len_N)), len_R))
```
...
for (data_set in data_sets) {
  ...
  for (rank in 1:len_R) {
    ...
    tse <- eval(parse(text = paste0("altExps(containers[[", cur_set, "]])$", custom_ranks[[rank]])))
    ...
    for (N in sample_sizes) {
      ...
      if (ncol(tse) >= N) {
        ...
        # test melting for tse
        df$MeltTime[tse_ind] <- melt_tse_exec_time(sub_tse)

        # test melting for pseq
        df$MeltTime[pseq_ind] <- melt_pseq_exec_time(sub_pseq)
