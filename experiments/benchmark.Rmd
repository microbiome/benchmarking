...
- [SongQAData](https://microbiome.github.io/microbiomeDataSets/reference/SongQAData.html)
  Song et al. (2016)
- HMP_2019_ibdmdb ([source 1](https://pubmed.ncbi.nlm.nih.gov/29311644/),
  [source 2](https://pubmed.ncbi.nlm.nih.gov/31142855/))
...
samples <- data.frame(Dataset = sapply(containers, mainExpName),
                      N = sapply(containers, ncol)) %>%
    arrange(N)
...
feat <- df_benchmark %>%
    select(Dataset, Rank, Features) %>%
    group_by(Dataset, Rank) %>%
    summarise(Features = max(Features)) %>% # Variation due to removing zero rows & cols per round
		unique() %>%
    pivot_wider(names_from = Dataset, values_from = Features, values_fill = 0)

feat <- feat[order(pull(feat, 2)), ] # Sort by the first dataset feature counts
kable(feat)
```
...
df <- df_benchmark %>%
    select(Dataset, Time, Features, Samples, Rank, ObjectType) %>%
...
df <- df %>%
    filter(Dataset %in% names(which(table(df$Dataset) >= 10))) %>%
